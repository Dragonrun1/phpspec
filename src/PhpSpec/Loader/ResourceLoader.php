...
class ResourceLoader
{
    ...
    public function load($locator, $line = null)
    {
        ...
        foreach ($this->manager->locateResources($locator) as $resource) {
            ...
            foreach ($reflection->getMethods(ReflectionMethod::IS_PUBLIC) as $method) {
                ...
                $example
                    = new Node\ExampleNode(ucfirst($this->smartExampleNameSplitter($method->getName())),
                    $method);
    ...
    /**
     * Detects underscore_case vs camelCase or PascalCase and splits into words
     * accordingly.
     *
     * @param $name
     *
    ...
    private function smartExampleNameSplitter($name)
    {
        ...
        $split = preg_split('/(^[^A-Z]+|[A-Z][^A-Z]+)/', $name, null,
